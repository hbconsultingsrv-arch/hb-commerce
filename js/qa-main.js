...
let qaCache = { requirements: [], tests: [], runs: [] };
...
async function qaReloadAll() {
  const [requirements, tests, runs] = await Promise.all([
    fetchQaRequirements(),
    fetchQaTestCases(),
    fetchQaTestRuns()
  ]);
  qaCache = { requirements, tests, runs };
  renderQaDashboard();
...
function renderQaAlerts() {
  ...
  if (!alerts.length) {
    host.innerHTML = '<div class="qa-alert qa-alert--info">✓ Aucune anomalie de couverture détectée.</div>';
    return;
  }
...
function renderQaDashboard() {
  ...
  set('qaKpiDeveloped', stats.developed);
  set('qaKpiValidated', stats.validated);
  set('qaKpiCoverage', `${stats.globalCoverage}%`);
...
async function runAllQaTests() {
  ...
  if (msg) msg.textContent = 'Démarrage du runner QA…';
  if (fill) fill.style.width = '5%';
  if (logsEl) logsEl.textContent = '';

  try {
    const { jobId } = await startQaTestRun();
...
function bindQaShell() {
  ...
  document.getElementById('runAllTestsBtn')?.addEventListener('click', runAllQaTests);
