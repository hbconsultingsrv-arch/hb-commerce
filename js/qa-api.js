...
function detectQaGaps(requirements, testCases, options = {}) {
  const { executions = [] } = options;
  const alerts = [];
  ...
  trace.forEach((row) => {
    ...
    if (row.developed && req.dev_status !== 'valide') {
      alerts.push({ type: 'not-validated', severity: 'warning', message: `⚠ ${req.req_id} est développée mais jamais validée.` });
    }
    if (row.failedCount) {
      alerts.push({ type: 'failed', severity: 'error', message: `⚠ ${req.req_id} : ${row.failedCount} test(s) en échec.` });
    }
    if (row.developed && !row.executedCount && row.testCount) {
      alerts.push({ type: 'not-tested', severity: 'info', message: `⚠ ${req.req_id} possède des tests mais aucun n'a été exécuté.` });
    }
  });
  ...
  testCases.forEach((t) => {
    ...
    if (t.status === 'echec') {
      const streak = countConsecutiveFailures(t.test_id, executions);
      if (streak >= 3) {
        alerts.push({
          type: 'streak-fail',
          severity: 'error',
          message: `⚠ ${t.test_id} échoue depuis ${streak} exécutions consécutives.`
        });
      }
    }
  });

  const uncovered = trace.filter((r) => !r.testCount && ['developpe', 'valide', 'en_cours'].includes(r.requirement.dev_status));
  if (uncovered.length) {
    alerts.push({
      type: 'uncovered-reqs',
      severity: 'warning',
      message: `⚠ ${uncovered.length} exigence(s) sans couverture de tests.`
    });
  }
...
function countConsecutiveFailures(testId, executions) {
  const sorted = (executions || [])
    .filter((e) => e.test_id === testId)
    .sort((a, b) => new Date(b.executed_at || 0) - new Date(a.executed_at || 0));
  let count = 0;
  for (const e of sorted) {
    if (e.status === 'echec') count++;
    else break;
  }
  return count;
}

function getQaRunConfig(testCases, mode = 'all') {
  if (mode === 'failed') {
    const files = [...new Set(
      testCases.filter((t) => t.status === 'echec' && t.playwright_file).map((t) => t.playwright_file)
    )];
    return { mode: 'failed', files, useLastFailed: !files.length };
  }
  if (mode === 'critical') {
    const files = [...new Set(
      testCases.filter((t) => t.priority === 'Critique' && t.playwright_file).map((t) => t.playwright_file)
    )];
    return { mode: 'critical', files };
  }
  return { mode: 'all', files: [] };
}

function computeTestsByModule(requirements, testCases) {
  const modules = {};
  testCases.forEach((t) => {
    const req = requirements.find((r) => r.id === t.requirement_id || r.req_id === t.req_ref);
    const mod = req?.category || 'Non classé';
    if (!modules[mod]) modules[mod] = { total: 0, passed: 0, failed: 0, pending: 0 };
    modules[mod].total++;
    if (t.status === 'succes') modules[mod].passed++;
    else if (t.status === 'echec') modules[mod].failed++;
    else modules[mod].pending++;
  });
  return Object.entries(modules).map(([label, s]) => ({ label, ...s }));
}

function detectNewFeaturesWithoutTests(roadmapItems, requirements, testCases) {
  if (!roadmapItems?.length) return [];
  const trace = computeQaTraceability(requirements, testCases);
  const coveredTitles = new Set(trace.filter((t) => t.testCount).map((t) => t.requirement.name.toLowerCase()));
  return roadmapItems
    .filter((item) => ['done', 'validated', 'in_progress'].includes(item.status))
    .filter((item) => {
      const title = (item.title || '').toLowerCase();
      const hasReq = requirements.some((r) =>
        r.name.toLowerCase().includes(title.slice(0, 12)) || title.includes(r.name.toLowerCase().slice(0, 12))
      );
      const hasTest = testCases.some((t) =>
        (t.title || '').toLowerCase().includes(title.slice(0, 10))
      );
      return !hasReq && !hasTest && !coveredTitles.has(title);
    })
    .map((item) => ({
      type: 'new-feature',
      severity: 'warning',
      message: `⚠ Fonctionnalité « ${item.title} » (${item.category || 'général'}) sans tests associés.`
    }));
}
...
async function startQaTestRun(options = {}) {
...
window.countConsecutiveFailures = countConsecutiveFailures;
window.getQaRunConfig = getQaRunConfig;
window.computeTestsByModule = computeTestsByModule;
window.detectNewFeaturesWithoutTests = detectNewFeaturesWithoutTests;
