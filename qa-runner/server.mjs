...
function runPlaywright(job, options = {}) {
  return new Promise((resolve) => {
    job.status = 'running';
    job.message = options.mode === 'failed'
      ? 'Ré-exécution des tests échoués…'
    ...
    if (options.useLastFailed) {
      args.push('--last-failed');
    } else if (options.files?.length) {
      options.files.forEach((f) => {
        const file = f.startsWith('tests/') ? f : `tests/${f}`;
        args.push(file);
      });
    }

    job.logs.push(`Commande : ${cmd} ${args.join(' ')}`);
...
app.post('/api/run', async (req, res) => {
  ...
  const options = {
    ...
    useLastFailed: !!req.body?.useLastFailed
  };
  const job = createJob();
  job.runMode = options.mode;
  res.json({ jobId: job.id, status: job.status, mode: options.mode });

  runPlaywright(job, options).catch((err) => {
    job.status = 'failed';
    job.message = err.message;
    job.finishedAt = new Date().toISOString();
  });
